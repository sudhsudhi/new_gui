...
from Tkinter import *
import ttk
from alclient import recordclient,conn_send,execute,recordclient
from subprocess import Popen,call,PIPE
from executer6 import ex,ex2,di,timeparse,linker,timedifference
abc = None
#socket can be called by self.socc
class mainclass():
    def __init__(self,master):
        ...
        self.button4 = ttk.Button(master, text="Disconnect", command=lambda:self.connect(),state=DISABLED)
        ...
        self.button3 = ttk.Button(master, text="Excecute", command=lambda:self.execute(),state = DISABLED)
...
class Record_Testcases:
    def __init__(self,master):
        ...
        self.button8 = Button(master, text="Start Recording", command=lambda:self.recording(),height=1,state=DISABLED)
   	...
   	#recordclient(self.socc,self.Entry.get())
        ...
        listbox1.grid(row=3,column=1,sticky=NSEW,columnspan=8)
	...
	for i in range(len(blink)-1): #adding timedifference
		if str(type(blink[i]))=="<type 'list'>":
			...
			#print blink[i][1],blink[k][1],blink[k]
                        self.listbox1.insert(0,blink[i][1],blink[k][1],blink[k])
	...
	for i in range(len(testcase)-1):
			...
			#print testcase[i]
                        self.listbox1.insert(0,testcase[i])
			
			#print blink[i+1]
                        self.listbox1.insert(0,blink[i+1])
			#print ' '
    ...
    def execute_testcase(self):
        root4=Toplevel(self.master,bg="white")
        root4 = self.root4
        #self.master=master
